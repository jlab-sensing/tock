...
/// Debug UART writer + verbose panic handler (enabled with `--debug-macro`).
...
mod panic_logging {
    ...
    /// Print over UART and blink the LED on panic.
    #[panic_handler]
    unsafe fn panic_fmt(info: &PanicInfo) -> ! {
        let rcc = stm32wle5jc::rcc::Rcc::new();
...
#[cfg(feature = "debug-macro")]
pub use panic_logging::WRITER;

/// Panic handler without `debug-macro`: reset the board.
#[cfg(not(feature = "debug-macro"))]
#[panic_handler]
pub unsafe fn panic_fmt(_info: &core::panic::PanicInfo) -> ! {
    cortexm4::support::reset()
}
